...
# 最常用的类，传入的实例的顺序就是之后进行前向计算的顺序(forward)
# 十分常用 方便 易用
class Sequential(Module):
    ...
    @overload
    def __init__(self, *args: Module) -> None: # 有众多参数 一个实例就是一个参数 args
        ...

    @overload
    def __init__(self, arg: 'OrderedDict[str, Module]') -> None: # 仅有一个参数 就是定义的OderDict有序字典
        ...

    def __init__(self, *args):
        super(Sequential, self).__init__()
        if len(args) == 1 and isinstance(args[0], OrderedDict): # 判断参数args的数量，以此来判定是不是OrderedDict;并且，同时用取args里边的第0个参数 判断是不是OderedDict 实例 (双重判断)
            for key, module in args[0].items(): # 对OrderedDict进行拆包
                self.add_module(key, module) # 使用add_module函数 对 self 添加key module
        else:
            for idx, module in enumerate(args):# 如果不是OrderedDict实例，则使用idx(0,1,2,3...) module进行 enumerate枚举拆包
                self.add_module(str(idx), module) # 使用add_module函数 对self 添加str(idx) module
...
# ModuleList 模块儿列表 不是 （一个列表，里面是由module构成的) | 其仍然是一个Module(因为它继承自Module),具有Module的所有的方法 [好处]
class ModuleList(Module):
    ...
    def insert(self, index: int, module: Module) -> None:
        ...
        for i in range(len(self._modules), index, -1):
            self._modules[str(i)] = self._modules[str(i - 1)] # 创建一个新的modules列表 将原来modules列表里的内容全部复制给新的列表 结束之后 新的module列表里会在最后多出一个空的位置 index
        self._modules[str(index)] = module # 将要插入的module放进空出来的index的位置
    # 也就是说，实现了随便插入到哪里都可以

    # 在module列表的末尾添加一个module
    def append(self, module: Module) -> 'ModuleList':
        ...
        Args:
            module (nn.Module): module to append
        """
        self.add_module(str(len(self)), module) # 使用了add_module函数
        return self

    # 使用迭代器来 在module的最后 添加modules
    def extend(self, modules: Iterable[Module]) -> 'ModuleList':
        ...
        """
        if not isinstance(modules, container_abcs.Iterable):      # 首先判断modules是不是Iterable迭代器实例
            raise TypeError("ModuleList.extend should be called with an "
                            "iterable, but got " + type(modules).__name__)
        offset = len(self) # offset 弥补
        for i, module in enumerate(modules):
            self.add_module(str(offset + i), module) # 使用for循环，add_module函数 在offset之后进行以此modules(Iterable)里面包含的迭代器
        return self
